<template>
    <div :style="{ width: width + 'px' }" class="b-input">
        ...
        <div class="ip-wrapper">
            <input autocomplete="off" @focus="focused" @focusout="focusout" tabindex="0" :id="unique"
                :style="{ width: ipWidth + 'px' }" class="input" type="text" :value="modelValue" :name="name"
                @input="$emit('update:modelValue', $event.target.value)" :class="error ? 'input--error' : ''"
                @mouseover="mouseOver" @mouseleave="mouseOut">
            <div v-if="(recommends.length && showAutocomplete) || (hoverRcm && recommends.length)" class="autoComplete">
                <ul>
                    <li v-for="(v, k) in recommends" @mouseover="mouseOverCpl" @mouseleave="mouseOutCpl" @click="selectedRecommend(v)" :key="k">{{ v }}</li>
                </ul>
            </div>
